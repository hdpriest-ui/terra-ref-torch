...
class StitchingDataset(Dataset):
    def __init__(self, data_directory, resize_height=None, resize_width=None):
        """
    ...
    def setup(self):
        ...
        required_dirs = ['input1', 'input2', 'shift', 'label']
        ...
        label_len = len(self.videos['label']['frame'])
        assert input1_len == input2_len == shift_len == label_len, \
            f"Mismatch in frame counts: input1={input1_len}, input2={input2_len}, shift={shift_len}, label={label_len}"
    ...
    def __getitem__(self, idx):
        ...
        input1 = np_load_frame(input1_path, None, None)
        input2 = np_load_frame(input2_path, None, None)

        # Load label
        label_path = self.videos['label']['frame'][idx]
        label = np_load_frame(label_path, self._resize_height, self._resize_width)
        
        # Load shift
        shift_path = self.videos['shift']['frame'][idx]
        shift = np_load_h(shift_path)
